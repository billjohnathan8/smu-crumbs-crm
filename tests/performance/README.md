...
- Verify admin credentials: `admin@crm.com` with `E2E_ADMIN_PASSWORD` in your environment (and `.env.local` for local runs; see repository `.env.example`). For JMeter, `agent-crud-workflow.jmx` reads `E2E_ADMIN_PASSWORD` via `${__ENV(E2E_ADMIN_PASSWORD)}`.
