...
CSV_HEADER = ["clientId", "transaction", "amount", "date", "status"]
...
def generate_valid_rows(args: argparse.Namespace, rng: random.Random) -> list[list[str]]:
...
def edge_case_rows() -> list[list[str]]:
    return [
        ["", "D", "100.00", "2026-02-01", "Completed"],  # missing clientId
        ["clt_edge", "X", "200.00", "2026-02-02", "Completed"],  # invalid transaction
        ["clt_edge", "W", "-50.00", "2026-02-03", "Pending"],  # negative amount
        ["clt_edge", "D", "75.00", "2026/02/04", "Completed"],  # invalid date format
        ["clt_edge", "D", "75.00", "2026-02-05", "Unknown"],  # invalid status
    ]
