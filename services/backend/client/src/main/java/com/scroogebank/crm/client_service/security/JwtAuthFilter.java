 ...
 * {@code anyRequest().authenticated()} rule then rejects them with 401. Requests that do
 * carry a valid token receive a fully-authenticated context so downstream controllers can
 * also call {@link RequestAuth#requireUser} without redundant validation overhead.</p>
 */
@Component
