...
def create_app(log_service: LogService | None = None) -> FastAPI:
    ...
    @app.post("/api/v1/logs", response_model=LogEventResponse, status_code=status.HTTP_201_CREATED)
    def create_log(request: LogEventRequest, service: LogService = Depends(get_log_service)) -> LogEventResponse:
        try:
            ...
            raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="failed to persist log event") from exc
