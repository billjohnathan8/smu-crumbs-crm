#!/usr/bin/env python3
"""Run the verification-service local test pipeline with a dedicated virtualenv."""
...
def main() -> int:
    ...
    # 2) Build (syntax compilation check)
    run(
        [
            str(venv_python),
            "-m",
    ...
    coverage_data_file = Path(tempfile.gettempdir()) / "verification-service-coverage"
    run(
        [
            str(venv_python),
            "-m",
