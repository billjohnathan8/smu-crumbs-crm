...
This service implements the user CRUD endpoints defined in
`docs/api-contracts/openapi/user.yaml`.
...
From the repo root:
```powershell
docker run --rm -p 5432:5432 -e POSTGRES_PASSWORD=postgres -e POSTGRES_DB=users postgres
```
...
- `DB_URL` (default: `jdbc:postgresql://localhost:5432/users`)
