...
test.describe('Agent View Transactions - Filters & Pagination (Flow 7)', () => {
    ...
    await test.step('Set up routes to capture filter params', async () => {
      await page.route('**/api/**', (route: Route) => {
        ...
        if (url.includes('/api/transactions')) {
          ...
          if (status) {
            filteredData = sampleTransactions.filter(t => t.status === status)
          }
  ...
  test('should filter transactions by type (Deposit/Withdrawal)', async ({ page }) => {
    let lastRequestParams: URLSearchParams | null = null
    ...
    await test.step('Set up routes', async () => {
      await page.route('**/api/**', (route: Route) => {
        ...
        if (url.includes('/api/transactions')) {
          ...
          if (transactionType) {
            filteredData = sampleTransactions.filter(t => t.transaction === transactionType)
          }
  ...
  test('should search transactions by client ID or transaction ID', async ({ page }) => {
    await test.step('Set up routes', async () => {
      await page.route('**/api/**', (route: Route) => {
        const url = route.request().url()
        ...
        if (url.includes('/api/transactions')) {
          return route.fulfill({
            ...
            body: JSON.stringify({
              ...
              pagination: { limit: 20, offset: 0, total: sampleTransactions.length },
            }),
          })
        }
    ...
    await test.step('Set up routes', async () => {
      await page.route('**/api/**', (route: Route) => {
        ...
        if (url.includes('/api/transactions')) {
          ...
          let filteredData = sampleTransactions
          if (status) filteredData = filteredData.filter(t => t.status === status)
          if (transaction) filteredData = filteredData.filter(t => t.transaction === transaction)
  ...
  test('should handle pagination', async ({ page }) => {
    ...
    await test.step('Verify page 2 content', async () => {
      ...
      await expect(page.getByText('Page 2 of 3')).toBeVisible({ timeout: 5000 })
    })
    ...
    await test.step('Verify back on page 1', async () => {
      ...
      await expect(page.getByText('Page 1 of 3')).toBeVisible({ timeout: 5000 })
    })
  })

  test('should display empty state when no transactions found', async ({ page }) => {
