...
test.describe('Admin Manage Accounts (Flow 3)', () => {
  ...
  test('should handle 409 conflict when creating duplicate user', async ({ page }) => {
  ...
  test('should disable user successfully', async ({ page }) => {
    ...
    await test.step('Set up routes', async () => {
      await page.route('**/api/**', (route: Route) => {
        ...
        if (url.includes('/api/agents/') && route.request().method() === 'PUT') {
    ...
    await test.step('Verify disable API call was made and success message shown', async () => {
      ...
      if (disableCallMade) {
        ...
        const successVisible = await page.getByText(/disabled successfully/i).isVisible().catch(() => false)
        if (successVisible) {
  ...
  test('should delete user successfully', async ({ page }) => {
    ...
    await test.step('Set up routes', async () => {
      await page.route('**/api/**', (route: Route) => {
        ...
        if (url.includes('/api/agents/') && route.request().method() === 'DELETE') {
          deleteCallMade = true
          return route.fulfill({
            status: 204,
            contentType: 'application/json',
  ...
  test('should reset user password successfully', async ({ page }) => {
    ...
    await test.step('Verify reset API call was made and success message shown', async () => {
      ...
      if (resetCallMade) {
        ...
        // Check for success message
        const successVisible = await page.getByText(/password reset email sent/i).isVisible().catch(() => false)
        if (successVisible) {
