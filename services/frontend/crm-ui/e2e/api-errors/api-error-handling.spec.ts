...
test.describe('API Error Handling (Flow 10)', () => {
  ...
  test('should display 403 Forbidden error when creating user without permission', async ({
    ...
    await test.step('Set up routes', async () => {
      await page.route('**/api/**', (route: Route) => {
        ...
        if (url.includes('/api/agents') && route.request().method() === 'POST') {
          return route.fulfill({
            status: 403,
            contentType: 'application/json',
            body: JSON.stringify({
    ...
    await test.step('Navigate to manage accounts and try to create user', async () => {
    ...
    await test.step('Verify 403 error is displayed', async () => {
      ...
      await expect(
        page.locator('text=/Insufficient permissions|forbidden|Failed to create/i')
  ...
  test('should handle 422 Validation error and display field errors', async ({ page }) => {
    ...
    await test.step('Verify 422 error message is displayed', async () => {
      await expect(
        page.locator('text=/Invalid data|validation|check your inputs/i')
      ).toBeVisible({ timeout: 5000 })
    })
  })
  ...
  test('should handle network failure (offline scenario)', async ({ page }) => {
    ...
    await test.step('Verify network error is shown', async () => {
      ...
      const hasError = await page.locator('text=/error|failed|unable|could not/i').first().isVisible().catch(() => false)
      const noData = await page.locator('text=/no (transactions|clients|data)|empty/i').first().isVisible().catch(() => false)
      const loading = await page.locator('text=/loading/i').first().isVisible().catch(() => false)
  ...
  test('should handle request timeout (5s)', async ({ page }) => {
    ...
    await test.step('Verify timeout handling (loading state persists or error shown)', async () => {
      ...
      const hasLoadingOrError =
        ...
        (await noData.first().isVisible().catch(() => false))
