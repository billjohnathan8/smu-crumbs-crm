...
describe('logs API', () => {
  ...
  describe('listClientLogs', () => {
    it('should list logs for a specific client without params', async () => {
      ...
      expect(client.apiGet).toHaveBeenCalledWith('/api/clients/client-123/logs')
      expect(result).toEqual(mockResponse)
    })
    ...
    it('should list client logs with limit and offset', async () => {
      ...
      expect(client.apiGet).toHaveBeenCalledWith('/api/clients/client-456/logs?limit=20&offset=40')
    })
  })
})
