...
describe("AgentViewTransactions", () => {
  ...
  it("should show loading state initially", () => {
    vi.spyOn(transactionsApi, "listTransactions").mockImplementation(
      () => new Promise(() => {}),
    );
  ...
  it("should render filter controls", async () => {
    ...
    expect(
      screen.getByPlaceholderText("Client ID or Transaction ID"),
    ).toBeInTheDocument();
    expect(
  ...
  it("should handle pagination when total exceeds page size", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/Showing 1 to 20 of 25 transactions/),
      ).toBeInTheDocument();
  ...
  it("should navigate to next page when clicking Next button", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/Showing 21 to 25 of 25 transactions/),
      ).toBeInTheDocument();
    });
  });
  ...
  it("should filter by status", async () => {
    ...
    const statusSelect =
      selects.find((select) =>
        select.getAttribute("class")?.includes("status"),
      ) || selects[0];
  ...
  it("should filter by transaction type", async () => {
    ...
    const selects = screen.getAllByRole("combobox");
    const typeSelect =
      selects.find((select) => select.textContent?.includes("All Types")) ||
      selects[1];
