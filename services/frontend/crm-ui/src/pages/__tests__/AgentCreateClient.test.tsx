...
describe("AgentCreateClient", () => {
  ...
  it("should render the create client form", () => {
    ...
    expect(
      screen.getByRole("heading", { name: /Create Client/i }),
    ).toBeInTheDocument();
    expect(
  ...
  it("should show validation error for client younger than 18", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText("Client must be at least 18 years old"),
      ).toBeInTheDocument();
    });
  });
  ...
  it("should show validation error for client older than 100", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText("Client age cannot exceed 100 years"),
      ).toBeInTheDocument();
    });
  });
  ...
  it("should accept exactly 18 years old", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.queryByText("Client must be at least 18 years old"),
      ).not.toBeInTheDocument();
      expect(
        screen.queryByText("Client age cannot exceed 100 years"),
      ).not.toBeInTheDocument();
    });
  });
  ...
  it("should show validation error for invalid phone format", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText("Invalid phone format (min 8 digits)"),
      ).toBeInTheDocument();
    });
  });
  ...
  it("should display API error for duplicate email (409)", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText("A client with this email already exists"),
      ).toBeInTheDocument();
    });
  });
