...
export function RootArchivedAdminsPage() {
  ...
  const load = async () => {
    try {
      setLoading(true)
      setError('')
      const response = await listArchivedUsers({ role: 'admin', limit: 200 })
      setUsers(response.data)
    } catch (err) {
      if (err instanceof ApiError) {
        if (err.status === 401) logout()
        else setError(err.message || 'Failed to load archived admins')
      } else {
  ...
  const handleReinstate = async (target: User) => {
    ...
    try {
      ...
      if (err instanceof ApiError) {
        if (err.status === 401) logout()
        else setError(err.message || 'Failed to reinstate archived admin')
      } else {
        setError('An unexpected error occurred')
      }
    } finally {
  ...
  return (
    <SidebarLayout items={getSidebarNavForUser(user)}>
      <div className="flex h-16 items-center justify-between">
        <h1 className="text-2xl font-normal text-text">Archived Admins</h1>
      </div>
      ...
      <main className="mt-6 space-y-6">
        ...
        {loading ? (
          <p className="text-text-muted">Loading archived admins...</p>
        ) : users.length === 0 ? (
          <p className="text-text-subtle">No archived admins found.</p>
        ) : (
          <div className="rounded-lg bg-card">
            <div className="flex items-center justify-between border-b border-border px-6 py-4">
              <h2 className="text-xl font-normal text-text">Archived Admins</h2>
