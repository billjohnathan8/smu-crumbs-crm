...
log_dir="${repo_root}/build-logs/build-and-test"
year="$(date '+%Y')"
month="$(date '+%m')"
day="$(date '+%d')"
hour="$(date '+%H')"
...
if [[ -n "${python_for_report}" && -f "${repo_root}/scripts/build-and-test/generate-coverage-index.py" ]]; then
  log "Generating aggregated coverage report (build-logs/build-and-test/index.html)"
  PYTHONDONTWRITEBYTECODE=1 BUILD_LOG_DIR="${log_dir}" "${python_for_report}" "${repo_root}/scripts/build-and-test/generate-coverage-index.py" || log "Coverage index generation failed."
else
